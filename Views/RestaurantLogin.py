...
def open_restaurant_login_window(db, event=None):
    ...
    login_button = ctk.CTkButton(main_frame, text="Iniciar Sesión", command=validation)  # Añade tu comando aquí
    ...
    register_label.bind("<Button-1>", lambda event: [BasicController.cancel_all_events(window),window.destroy(), open_restaurant_registration_window(db)])
